...
namespace FlacDecode.MathCoding
{
	public class LinearPredictiveCoding
	{
		...
		public const int MaxLpcWindows = 2;
		...
		public static unsafe void window_flattop(float* window, int L)
		{
			int N = L - 1;
			for (int n = 0; n < L; n++)
				window[n] = (float) (1.0 - 1.93*System.Math.Cos(2.0*System.Math.PI*n/N) + 1.29*System.Math.Cos(4.0*System.Math.PI*n/N) - 0.388*System.Math.Cos(6.0*System.Math.PI*n/N) + 0.0322*System.Math.Cos(8.0*System.Math.PI*n/N));
		}

		public static unsafe void window_tukey(float* window, int L)
		{
			...
			if (Np > 0)
			{
				for (int n = 0; n <= Np; n++)
				{
					window[n] = (float) (0.5 - 0.5*System.Math.Cos(System.Math.PI*n/Np));
					window[L - Np - 1 + n] = (float) (0.5 - 0.5*System.Math.Cos(System.Math.PI*(n + Np)/Np));
				}
			}
		}
		...
		public static unsafe void window_hann(float* window, int L)
		{
			int N = L - 1;
			for (int n = 0; n < L; n++)
				window[n] = (float) (0.5 - 0.5*System.Math.Cos(2.0*System.Math.PI*n/N));
		}
		...
		{
			double* lpc_tmp = stackalloc double[MaxLpcOrder];

			if (max_order > MaxLpcOrder)
		...
		{
			double* gen0 = stackalloc double[MaxLpcOrder];
			double* gen1 = stackalloc double[MaxLpcOrder];
			                   ...
			                   out int shift, int max_shift, int zero_shift)
		{
			...
			int qmax = (1 << ((int) precision - 1)) - 1;

			// find maximum coefficient value
			float cmax = 0.0F;
			for (i = 0; i < order; i++)
			{
				float d = System.Math.Abs(lpc_in[i]);
				if (d > cmax)
					cmax = d;
			}
